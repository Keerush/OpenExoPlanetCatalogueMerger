		...
		request({
			...
			if (!err && response.statusCode === 200) {
				...
				data.forEach(function(item) {
					systemInput.push([item.pl_hostname, formatDistance(item.dec_str, ['d', 'm', 's']), formatDistance(item.ra_str, ['h', 'm', 's']), item.st_dist, item.st_disterr1, item.st_disterr2]);
				...
				var systemQuery = 'REPLACE INTO NasaSystem (name, declination, rightascension, distance, distanceplus, distanceminus) VALUES ?',
